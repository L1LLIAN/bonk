...
public class CommandParameters {
    ...
    public CommandParameters(CommandRegistry service, Method method) {
        CommandParameter[] parameters = new CommandParameter[method.getParameters().length];
        for (int i = 0; i < method.getParameters().length; i++) {
            Parameter param = method.getParameters()[i];
            parameters[i] = new CommandParameter(service, param.getType(), param, method.getParameterAnnotations()[i]);
        }
        this.parameters = parameters;
